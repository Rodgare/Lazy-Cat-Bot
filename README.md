# 🐈 LazyCatBot — Дискорд бот для отслеживания новых убийств боссов вашей гильдией и не только
...
2. В этом канале введите команду `/menu`, появиться кнопка "Добавить гильдию", выберите сервер и введите ID гильдии.
3. Бот сразу начнет мониторинг! Если бот начал отправлять килы гильдии за текущее кд, значит все ок и бот работает. (отправку отчетов об убийствах можно отключить в /menu)
> ID гильдии можно найти на сайте sirus.su, на странице вашей ги в URL адрессе последнее число это ваш ID. Например https://sirus.su/base/guilds/x3/913 ID это 913.
...
(кнопки можно закрепить в канале для удобства, взаимодействовать с ними могут все, первый ряд дпс, второй ряд хпс)
...
Проверьте, есть ли у бота права на просмотр данного канала и на отправку сообщений (Embed).
Проверьте включены ли отправки отчетов в меню /menu должно быть "Отчеты: ВКЛЮЧЕНЫ" как на скрине ниже:

![Checkboss](img/reports_enabled.png)